...
public class ModelDelegateTests
{
	...
	public void testMergeSimpleScanWithBiDirectionalStrategy() throws Exception
	{
		try
		{
			...
			Scan scan = modelDelegate.performScan(scanDefinition, threadPool, testHarness.getScanOptions());
			// Bidirectional merge prefers the newer mtime (ties favor source). Artifact
			// mtimes from checkout/copy are nondeterministic, so pin ordering before merge.
			long t = System.currentTimeMillis();
			assertTrue(testFile1.setLastModified(t));
			assertTrue(testFile2.setLastModified(t - 10_000L));
			scan = modelDelegate.mergeScan(scan, threadPool);
			assertEquals(1, scan.getUnmodifiableResults().size());
			for (ComparisonResult result : scan.getUnmodifiableResults())
			{
	...
	/**
	 * Regression test for the {@code IllegalArgumentException} surfaced by
	 * the T3.8 IPC restore smoke. {@code FileUtils.moveFile} rejects
	 * directories, so {@code MergeEngine.restoreDeletedItem} must dispatch
	 * to {@code moveDirectory} when the staged item is a directory.
	 *
	 * <p>This is a focused unit test of the merge engine's restore branch;
	 * it bypasses {@code ModelDelegate.restoreDeletedItem} (which would
	 * require persisting a Scan + ScanDefinition to satisfy the
	 * {@code entityManager.flush()} at the end of the transactional method)
	 * because the bug under test is purely in the file-move dispatch.
	 */
	@Test
	public void testMergeEngineRestoresDeletedDirectory() throws Exception
	{
		File sourceDir = new File(testHarness.getTestArtifactLocation()
			+ File.separator + "restore_dir_src" + File.separator + "tree");
		File targetDir = new File(testHarness.getTestArtifactLocation()
			+ File.separator + "restore_dir_tgt" + File.separator + "tree");
		FileUtils.forceMkdir(sourceDir);
		FileUtils.forceMkdir(targetDir);
		FileUtils.writeStringToFile(new File(sourceDir, "child.txt"), "x", java.nio.charset.StandardCharsets.UTF_8);
		FileUtils.writeStringToFile(new File(targetDir, "child.txt"), "x", java.nio.charset.StandardCharsets.UTF_8);

		// A parent Scan + ScanDefinition is required because the staged
		// path (Scan.getScanDeletedItemsLocation) is derived from the
		// scan-definition name and the scan's start time. Neither needs to
		// be persisted for the merge-engine's file-move branch to run.
		ScanDefinition scanDefinition = new ScanDefinition();
		scanDefinition.setName("restore_dir_test");
		Scan scan = new Scan();
		scan.setScanDef(scanDefinition);

		ComparisonResult result = new ComparisonResult();
		result.setSource(sourceDir);
		result.setTarget(targetDir);
		result.setResolution(AssuranceResultResolution.DELETE_SOURCE);
		scan.addResult(result);

		testHarness.buildDeletedItemsStructure(result);
		assertTrue(!sourceDir.exists());
		assertTrue(result.getSourceDeletedItemLocation(MergeEngine.getApplicationDeletedItemsLocation()).isDirectory());

		// TargetMergeEngine is the strategy ModelDelegate.restoreDeletedItem
		// chooses for DELETE_SOURCE — see ModelDelegate.java:440-447.
		new com.markallenjohnson.assurance.model.merge.TargetMergeEngine()
			.restoreDeletedItem(result, null);

		assertEquals(AssuranceResultResolution.UNRESOLVED, result.getResolution());
		assertTrue(sourceDir.isDirectory());
		assertTrue(new File(sourceDir, "child.txt").isFile());
	}
