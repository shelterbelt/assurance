...
import com.markallenjohnson.assurance.model.compare.IComparisonEngine;
...
import com.markallenjohnson.assurance.model.factories.IMergeEngineFactory;
...
public class ModelDelegate implements IModelDelegate
{
	...
	@Transactional
	public Scan openScan(ScanDefinition scanDefinition)
	{
		if (scanDefinition == null)
		{
			throw new IllegalArgumentException("scanDefinition must not be null");
		}
		ScanDefinition managedDefinition = scanDefinition;
		if (!entityManager.contains(managedDefinition))
		{
			managedDefinition = entityManager.merge(managedDefinition);
		}
		Scan scan = new Scan();
		scan.setScanDef(managedDefinition);
	...
	public Scan getScanById(Long id)
	{
		...
		return entityManager.find(Scan.class, id);
	}

	@Transactional(readOnly = true)
	@Override
	public int countComparisonResultsForScan(Long scanId)
	{
		if (scanId == null)
		{
			return 0;
		...
		Long count = this.entityManager
			.createQuery("SELECT COUNT(r) FROM ComparisonResult r WHERE r.scan.id = :scanId", Long.class)
	...
	@Transactional(readOnly = true)
	@Override
	public int countResolvedComparisonResultsForScan(Long scanId)
	{
		if (scanId == null)
		{
			return 0;
		...
		Long count = this.entityManager
			.createQuery(
				"SELECT COUNT(r) FROM ComparisonResult r WHERE r.scan.id = :scanId AND r.resolution <> :unresolved",
				Long.class)
			.setParameter("scanId", scanId)
			.setParameter("unresolved", AssuranceResultResolution.UNRESOLVED)
			.getSingleResult();
		return count == null ? 0 : count.intValue();
	}

	@Transactional
	public Scan completeScan(Long scanId, IAssuranceThreadPool threadPool, IScanOptions options, IProgressMonitor monitor) throws AssuranceNullFileReferenceException, AssuranceIncompleteScanDefinitionException
	{
		if (scanId == null)
		{
			throw new IllegalArgumentException("scanId must not be null");
		}
		Scan scan = entityManager.find(Scan.class, scanId);
		if (scan == null)
		{
		...
		{
			throw new AssuranceIncompleteScanDefinitionException("No scan definition is bound to scan " + scanId);
		}
		...
		// Force-load the lazy results collection so callers (e.g. the IPC
		// layer that emits the scanCompleted notification) can read its size
		// after the transaction closes without triggering a
	...
	{
		ScanDefinition managed = scanDefinition;
	...
	@Transactional
	public ScanDefinition getScanDefinitionById(Long id)
	{
		if (id == null)
		{
			return null;
		}
		return entityManager.find(ScanDefinition.class, id);
	}
	...
	public ComparisonResult getResultById(Long id)
	{
		...
		return entityManager.find(ComparisonResult.class, id);
	}
	...
	public ApplicationConfiguration saveApplicationConfiguration(ApplicationConfiguration config)
	{
		...
		entityManager.flush();
		// Re-derive the parsed list views from the persisted scalar columns so
		// the returned instance matches what a fresh getApplicationConfiguration()
		// would produce. The IPC layer projects the list views onto the wire.
		config.initialize();
		return config;
	}
