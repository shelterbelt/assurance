...
public class MergeWorker implements Runnable
{
	...
	public void run()
	{
		...
		{
			// submit() always called register(); unregister must match it
			// unconditionally. The prior 'scan != null' guard leaked Phaser
			// parties whenever result.getScan() was null, which inflated the
			// pool's unarrived counter and caused await() to block forever
			// (masked previously by a counter-leak in await() itself).
