...
public class FileReference
{
	...
	/**
	 * Re-materialises {@link #fileAttributes} from {@link #getFile()} on disk,
	 * preserving the existing row when one is already associated. Used after
	 * merge/restore so attribute columns are not left stale from the scan that
	 * created the comparison row.
	 */
	public void refreshStoredFileAttributes()
	{
		...
		IFileComparer comparer = this.getComparer();
		if (this.fileAttributes != null)
		{
			this.fileAttributes.refreshFrom(file, comparer);
		}
		else
		{
			this.setFileAttributes(new FileAttributes(file, comparer));
		}
	}
